        ...
        <div className="space-y-4 sm:space-y-6">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 backdrop-blur-sm p-4">
                    <div className={cn('rounded-xl sm:rounded-2xl p-4 sm:p-6 w-full max-w-md shadow-2xl border transition-colors', isDark ? 'bg-[#1e1e1e] border-gray-800' : 'bg-white border-gray-200')}>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className={cn('block text-sm font-medium mb-1.5', isDark ? 'text-gray-400' : 'text-gray-600')}>Service Name</label>
                                <input required type="text" className={cn('w-full border rounded-xl p-3 outline-none focus:border-[#ff7a6b] transition-colors text-sm', isDark ? 'bg-[#0f0f0f] border-gray-800 text-white' : 'bg-white border-gray-200 text-gray-900')} value={newService.name} onChange={(e) => setNewService({ ...newService, name: e.target.value })} placeholder="e.g. Consultation" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className={cn('block text-sm font-medium mb-1.5', isDark ? 'text-gray-400' : 'text-gray-600')}>Price (Rs)</label>
                                    <input required type="number" className={cn('w-full border rounded-xl p-3 outline-none focus:border-[#ff7a6b] transition-colors text-sm', isDark ? 'bg-[#0f0f0f] border-gray-800 text-white' : 'bg-white border-gray-200 text-gray-900')} value={newService.price} onChange={(e) => setNewService({ ...newService, price: e.target.value })} placeholder="0.00" />
                                </div>
                                <div>
                                    <label className={cn('block text-sm font-medium mb-1.5', isDark ? 'text-gray-400' : 'text-gray-600')}>Duration (min)</label>
                                    <input required type="number" className={cn('w-full border rounded-xl p-3 outline-none focus:border-[#ff7a6b] transition-colors text-sm', isDark ? 'bg-[#0f0f0f] border-gray-800 text-white' : 'bg-white border-gray-200 text-gray-900')} value={newService.duration} onChange={(e) => setNewService({ ...newService, duration: e.target.value })} placeholder="30" />
                                </div>
                            </div>
                            <div className="flex flex-col sm:flex-row justify-end gap-3 mt-6">
                                <button type="button" onClick={() => setIsModalOpen(false)} className={cn('w-full sm:w-auto px-4 py-2.5 rounded-xl transition-colors text-sm font-medium', isDark ? 'text-gray-400 hover:text-white hover:bg-white/5' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100')}>Cancel</button>
                                <button type="submit" className="w-full sm:w-auto px-6 py-2.5 bg-[#ff7a6b] text-white rounded-xl font-bold hover:bg-[#ff6b5b] transition-all active:scale-[0.98] shadow-lg shadow-[#ff7a6b]/20">Add Service</button>
                            </div>
                        </form>
                    </div>
                </div>
