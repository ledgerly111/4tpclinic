            ...
            <div className={cn('rounded-xl sm:rounded-2xl p-3 sm:p-4', isDark ? 'bg-[#1e1e1e]' : 'bg-white border border-gray-200')}>
                ...
                <div className="flex justify-between gap-1 sm:gap-2 overflow-x-auto pb-2">
                    {currentWeek.map((date, idx) => {
                        ...
                        return (
                            <button key={idx} onClick={() => setSelectedDate(date)} className={`flex flex-col items-center p-2 sm:p-3 rounded-xl transition-all min-w-[40px] sm:min-w-[60px] ${
                                    isSelected ? 'bg-[#ff7a6b] text-white' : isToday ? 'bg-[#ff7a6b]/20 text-[#ff7a6b]' : isDark ? 'bg-[#0f0f0f] text-gray-400 hover:bg-[#252525]' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'
                                }`}>
            ...
            <div className={cn('rounded-xl sm:rounded-2xl p-4 sm:p-6', isDark ? 'bg-[#1e1e1e]' : 'bg-white border border-gray-200')}>
                ...
                <div className="space-y-3">
                    {appointments.map((apt) => (
                        <div key={apt.id} className={cn('flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl transition-colors group', isDark ? 'bg-[#0f0f0f] hover:bg-[#151515]' : 'bg-gray-50 hover:bg-gray-100')}>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
                    <div className="bg-[#1e1e1e] rounded-2xl p-6 w-full max-w-md border border-gray-800">
                        <div className="flex items-center justify-between mb-4"><h2 className="text-white text-lg font-semibold">Create Appointment</h2><button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-white"><X className="w-5 h-5" /></button></div>
                        <form onSubmit={handleCreate} className="space-y-3">
                            <input required value={form.patientName} onChange={(e) => setForm({ ...form, patientName: e.target.value })} className="w-full rounded-xl bg-[#0f0f0f] border border-gray-800 p-3 text-white" placeholder="Patient Name" />
                            <input required value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })} className="w-full rounded-xl bg-[#0f0f0f] border border-gray-800 p-3 text-white" placeholder="Visit Type" />
                            <input value={form.doctor} onChange={(e) => setForm({ ...form, doctor: e.target.value })} className="w-full rounded-xl bg-[#0f0f0f] border border-gray-800 p-3 text-white" placeholder="Doctor" />
                            <input required type="time" value={form.time} onChange={(e) => setForm({ ...form, time: e.target.value })} className="w-full rounded-xl bg-[#0f0f0f] border border-gray-800 p-3 text-white" />
                            <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="w-full rounded-xl bg-[#0f0f0f] border border-gray-800 p-3 text-white" rows="3" placeholder="Notes" />
                            <button className="w-full rounded-xl bg-[#ff7a6b] py-2.5 text-white hover:bg-[#ff6b5b]">Create</button>
                        </form>
                    </div>
                </div>
            )}
