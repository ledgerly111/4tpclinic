...
function extractErrorMessage(error) {
  try {
    ...
    let causeMessage = "";
    try {
      causeMessage = error.cause && typeof error.cause.message === "string" ? error.cause.message : "";
    } catch {
      causeMessage = "";
    }
...
async function resolveUniqueInvoiceNumber(db, requestedNumber) {
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    const existing = await db.prepare("SELECT id FROM invoices WHERE invoice_number = ? LIMIT 1").bind(candidate).first();
    if (!existing) {
      return candidate;
    }
  }
...
__name(resolveUniqueInvoiceNumber, "resolveUniqueInvoiceNumber");
...
// .wrangler/tmp/bundle-svDPAr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-svDPAr/middleware-loader.entry.ts
